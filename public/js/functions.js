(function($){
		...
		$(document).on("ready",function() {    
			...
			var drop;
			if(["/sobre","/antecedentes","/propuestas","/acciones"].indexOf(path) != -1)
				drop = ".quienes";
			else if(["/flujos","/principios"].indexOf(path) != -1)
				drop = ".sobre";
			else if(["/documentos","/libros","/articulos","/revistas","/tesis"].indexOf(path) != -1)
				drop = ".referencias"
			else if(["/boletines","/comunicados","/cuadernos"].indexOf(path) != -1)
				drop = ".publicaciones";
			else if(["/capsulas","/documentales","/programas","/conferencias"].indexOf(path) != -1)
				drop = ".videoteca";
